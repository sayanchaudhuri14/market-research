...
Key change vs v2:
  ...
  NKD=F D-1 close.  sgx_ret = (N225_open_today - N225_close_prev) / N225_close_prev
  Fallback: if today's N225 open is unavailable, uses D-1 ^N225 close-to-close return.
...
Cron entry (EC2, Asia/Kolkata timezone — skip Mon = days 2-5):
  25 9 * * 2-5  python3 /path/to/cron_gap_trading/v4.3/entry.py >> /path/logs/cron_v43.log 2>&1
"""
...
def fetch_global_data(today: datetime.date) -> dict:
    ...
    v4.3 change: SGX signal is computed from ^N225 trade-day Open vs D-1 Close.
    Tokyo opens at 09:00 JST = 05:30 IST, so by 9:25 IST the N225 open is
    already 4 hours old — no lookahead.
    Fallback: if today's N225 open is unavailable, uses D-1 close-to-close return.
    """
    from datetime import timedelta
    start = str(today - timedelta(days=12))
    end   = str(today + timedelta(days=1))   # include today's bar for N225 open

    # Non-SGX tickers — only keep sessions that have closed (before today)
    TICKERS = {
        'SP500': '^GSPC',
        'DAX':   '^GDAXI',
        'VIX':   '^VIX',
    ...
    for name, ticker in TICKERS.items():
        try:
            ...
            df = df[df.index.date < today]   # only closed sessions
            raw[name] = df
        except Exception as e:
            print(f"  [warn] {name} fetch failed: {e}")
            raw[name] = pd.DataFrame()

    # ── N225: fetch including today to get trade-day Open ──────────────────────
    sgx_ret    = None
    sgx_source = 'unavailable'
    try:
        n225 = yf.download('^N225', start=start, end=end, progress=False, auto_adjust=True)
        if isinstance(n225.columns, pd.MultiIndex):
            n225.columns = n225.columns.get_level_values(0)
        if n225.index.tz is None:
            n225.index = n225.index.tz_localize('UTC')

        # Rows up to and including today
        n225_past  = n225[n225.index.date < today]   # D-1 and earlier
        n225_today = n225[n225.index.date == today]  # today's bar (if available)

        if len(n225_past) >= 2:
            n225_close_prev = float(n225_past['Close'].iloc[-1])  # D-1 close
            last_n225_date  = n225_past.index[-1].date()

            if (today - last_n225_date).days <= MAX_STALE_DAYS:
                if len(n225_today) > 0:
                    # Primary: use today's Open (Tokyo 09:00 JST ≈ 05:30 IST)
                    n225_open_today = float(n225_today['Open'].iloc[0])
                    sgx_ret    = (n225_open_today - n225_close_prev) / n225_close_prev
                    sgx_source = f'N225 open today ({n225_open_today:,.0f}) vs prev close ({n225_close_prev:,.0f})'
                else:
                    # Fallback: D-1 close-to-close return
                    n225_close_d2 = float(n225_past['Close'].iloc[-2])
                    sgx_ret    = (n225_close_prev - n225_close_d2) / n225_close_d2
                    sgx_source = f'N225 D-1 close ({n225_close_prev:,.0f}) — today open unavailable'
    except Exception as e:
        print(f"  [warn] ^N225 fetch failed: {e}")
        sgx_source = f'error: {e}'

    print(f"  N225 source       : {sgx_source}")
...
def compute_signals(gd: dict, nifty_open_915: Optional[float]) -> dict:
    ...
    SGX UP/DOWN now reflects N225 trade-day open direction (v4.3).
    """
...
def main():
    ...
    missing = [k for k, v in {
        'SP500': gd['sp500_ret'], 'SGX(N225)': gd['sgx_ret'],
    ...
    for k, v in [('S&P 500', gd['sp500_ret']), ('N225 Open (5:30 IST)', gd['sgx_ret']),
                 ('DAX', gd['dax_ret']), ('VIX', gd['vix_ret'])]:
        print(f"  {k:<22}: {v:>+10.2%}" if v is not None else f"  {k:<22}:    MISSING")
    ...
    pos = {
        ...
        "status":        "open",
        "version":       "v4.3",
        "sgx_signal_source": "N225_open_today",
    }
