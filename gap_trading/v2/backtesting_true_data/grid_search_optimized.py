...
# ── Grid to sweep ─────────────────────────────────────────────────────────────
SL_GRID   = [0.15, 0.20, 0.25, 0.30]
TP_GRID   = [0.40, 0.80, 1.00]
FIXED_GRID     = [5, 10]
...
top_bearish = (reliable[reliable['P_Down'] > BASE_RATE]
               .sort_values('Edge_pp', ascending=False).head(10).reset_index(drop=True))
...
for _, trow in tradeable_df.iterrows():
    ...
    day_min = (minute_all[minute_all.index.date == d].between_time('09:26', '11:15')
...
def simulate_sl_tp(sl_pct, tp_pct):
    ...
    for _, trow in tradeable_df.iterrows():
        ...
        if exit_price is None:
            ...
            exit_price = bs_price(spot_last, strike, tte_fn(dtime(11, 15)), RISK_FREE_RATE, iv, opt)
...
output_path = OUTPUT_DIR / 'grid_search_optimized.xlsx'
