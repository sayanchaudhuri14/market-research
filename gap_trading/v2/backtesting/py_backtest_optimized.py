...
for _, r in top_bearish.iterrows():
    print(
        ...
        f'P(DOWN)={r["P_Down"]:.1f}%  Edge=+{r["Edge_pp"]:.1f}%  N={int(r["N"])}'
    )
...
for _, r in top_bullish.iterrows():
    print(
        f'  [{int(r["Level"])}] {r["Signal"]:<55} '
        f'P(UP)={100 - r["P_Down"]:.1f}%  Edge=+{abs(r["Edge_pp"]):.1f}%  N={int(r["N"])}'
    )
...
def simulate_trade(trade_date, action, nifty_open, vix_india, minute_df):
    ...
    entry_price = bs_price(nifty_open, strike, tte(dtime(9, 25)), RISK_FREE_RATE, iv, opt_type)
    if entry_price < 0.5:
        return None
    ...
    day_min = day_min.between_time("09:26", "11:15")
    ...
    if exit_price is None:
        ...
        exit_price = bs_price(spot_last, strike, tte(dtime(11, 15)), RISK_FREE_RATE, iv, opt_type)
...
if len(results_df) > 0:
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "backtest_charts_optimized.png", dpi=150, bbox_inches="tight")
    plt.show()

# ── FINAL: Save everything into ONE Excel file ────────────────────────────────

output_file = OUTPUT_DIR / "backtest_report_optimized.xlsx"
