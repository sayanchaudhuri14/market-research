...
- BCS (DTE1-4) + Strip DTE4 strategies with optimised SL/TP from backtesting
...
- Reads Kite token from Authorize_Kite/kite_token.json (written by kite_auth cron at 9:15 AM)
...
# ── NSE Holidays 2025-2026 (verify variable-date holidays against NSE calendar) ─
NSE_HOLIDAYS = {
    # 2025
    ...
    # 2026 — variable religious holidays (approximate; verify on NSE website)
    date(2026, 2, 17),   # Mahashivratri
    date(2026, 3,  3),   # Holi
    date(2026, 3, 20),   # Eid al-Fitr (moon-dependent)
    date(2026, 5, 27),   # Eid al-Adha (moon-dependent)
...
# ── Strategy definitions (optimised SL/TP from backtesting) ───────────────────
STRATEGIES = [
    {'id': 'BCS_DTE2',  'type': 'BCS',   'dte': 2, 'sl': 0.50, 'tp': 1.00, 'span': 25_000, 'max_lots': 5, 'name': 'Bear Call Spread DTE2'},
    {'id': 'BCS_DTE3',  'type': 'BCS',   'dte': 3, 'sl': 0.50, 'tp': 1.00, 'span': 25_000, 'max_lots': 5, 'name': 'Bear Call Spread DTE3'},
    {'id': 'BCS_DTE4',  'type': 'BCS',   'dte': 4, 'sl': 0.75, 'tp': 0.75, 'span': 25_000, 'max_lots': 5, 'name': 'Bear Call Spread DTE4'},
    {'id': 'STRIP_DTE4','type': 'STRIP', 'dte': 4, 'sl': 1.00, 'tp': 0.25, 'span': 25_000, 'max_lots': 5, 'name': 'Strip DTE4'},
]

# ── Kite client ────────────────────────────────────────────────────────────────
def get_kite() -> Optional[KiteConnect]:
    """Read today's token from Authorize_Kite (written by kite_auth.py cron at 9:15 AM)."""
    try:
        return _kite_auth.get_kite(force_refresh=False)
    except Exception as e:
        log.error(f"Failed to get Kite client: {e}")
        return None
