...
# ── Kite client (read-only — never triggers a new login) ──────────────────────
def get_kite() -> Optional[KiteConnect]:
    """
    Reads today's access token from the shared kite_token.json written by
    the gap trading cron. Returns None (with a clear log message) if:
      - the file doesn't exist yet (gap trading cron hasn't run today)
      - the token is from a previous day (stale)
    Never attempts a fresh login — that's the gap trading cron's job.
    """
    if not KITE_TOKEN_PATH or not KITE_TOKEN_PATH.exists():
        log.error(
            f"Token file not found: {KITE_TOKEN_PATH}\n"
            "  → Gap trading cron must run first to create today's token.\n"
            "  → Check KITE_TOKEN_PATH in .env"
        )
        return None
    ...
    if cached.get('date') != today:
        log.error(
            f"Token is stale (dated {cached.get('date')}, today is {today}).\n"
            "  → Gap trading cron has not refreshed the token yet today.\n"
            "  → Paper trader will retry — ensure gap trading cron runs before 09:25."
        )
        return None

    kite = KiteConnect(api_key=KITE_API_KEY)
    kite.set_access_token(cached['access_token'])
    log.info(f"Kite authenticated via shared token (dated {today})")
    return kite
